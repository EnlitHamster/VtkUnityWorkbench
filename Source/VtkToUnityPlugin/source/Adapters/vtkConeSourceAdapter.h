...
class VtkConeSourceAdapter : VtkToUnityAdapter
{
public:
	VtkConeSourceAdapter();
	virtual ~VtkConeSourceAdapter() { }

	virtual void SetAttribute(vtkSmartPointer<vtkActor> actor, LPCSTR propertyName, LPCSTR newValue);
	virtual void GetAttribute(vtkSmartPointer<vtkActor> actor, LPCSTR propertyName, char* retValue);

protected:
	// Mapping the sources attributes to the specific getter and setter methods
	static const std::map<LPCSTR, std::pair<getter<VtkConeSourceAdapter>, setter<VtkConeSourceAdapter>>> s_attributes;
